...
studyDesign = data.frame(sample = as.factor(full_metadata$sample), site_spatial = as.factor(full_metadata$Site))
...
m.spatial = Hmsc(Y=yprob, XData=XData, XFormula=~.,
...
pdf("HMSC/Nf_Nd_variance_partitioning.bin.spatial.pdf", width = 8, height = 8)
p1
dev.off()
