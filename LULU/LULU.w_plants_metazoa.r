...
print(summary(lulu.asv_0.90))


#get all parents
parents = (lulu.asv_0.90$otu_map %>% filter(curated == "merged"))$parent_id %>% unique
#get all merged stats. need to do both manipulations to retain rownames
merged_asvs = data.frame(child = rownames(lulu.asv_0.90$otu_map),lulu.asv_0.90$otu_map)  %>% filter(parent_id %in% parents & curated != "parent")
...
#write.table(merged_asvs_taxonomy_and_sim, file = "LULU/merged_asvs_90.txt", row.names = F, sep = "\t", quote = F)
#write.table(lulu.asv_0.90$curated_table, file = "LULU/asv_tab.LULU_90.txt", sep = "\t", quote = F)
...
print(summary(lulu.asv_0.93))


#get all parents
parents = (lulu.asv_0.93$otu_map %>% filter(curated == "merged"))$parent_id %>% unique
#get all merged stats. need to do both manipulations to retain rownames
merged_asvs = data.frame(child = rownames(lulu.asv_0.93$otu_map),lulu.asv_0.93$otu_map)  %>% filter(parent_id %in% parents & curated != "parent")
...
#write.table(merged_asvs_taxonomy_and_sim, file = "LULU/merged_asvs_93.txt", row.names = F, sep = "\t", quote = F)
#write.table(lulu.asv_0.93$curated_table, file = "LULU/asv_tab.LULU_93.txt", sep = "\t", quote = F)
...
ASVs_LULU_93 = left_join(
data.frame(ASV = lulu.asv_0.93$curated_otus),
data.frame(ASV = row.names(asv_tax), asv_tax),
by = "ASV"
) %>%
group_by(Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
...
ASVs_LULU_90 = left_join(
data.frame(ASV = lulu.asv_0.90$curated_otus),
data.frame(ASV = row.names(asv_tax), asv_tax),
by = "ASV"
) %>%
group_by(Kingdom, Phylum, Class, Order, Family, Genus, Species) %>%
...
ASV_OTU_count_by_taxon = full_join(ASV_count_by_taxon, ASVs_LULU_95) %>%
    full_join(., ASVs_LULU_93) %>%
    full_join(., ASVs_LULU_90) %>%
    full_join(., ASVs_LULU_84)

write.table(ASV_OTU_count_by_taxon, file = "LULU/ASV_count_by_taxon.w_plants_metazoa.txt", row.names = F, quote = F, sep = "\t")
