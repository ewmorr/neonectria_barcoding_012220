...
taxa.w_bootstraps = readRDS("taxa_w_bootstraps.rds")
...
out2 = readRDS("read_filtering_read_counts_2.rds")
...
full_join(., readRDS("denoisedF.getN.df.rds"), by = "sample") %>%
full_join(., readRDS("denoisedR.getN.df.rds"), by = "sample") %>%
full_join(., readRDS("mergers.getN.df.rds"), by = "sample") %>%
