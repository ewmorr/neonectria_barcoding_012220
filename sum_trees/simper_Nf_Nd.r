...
#site
#simper(comm = log10(asv_tab.gt1K.rare+1) ~ full_meatadata.sorted$Site)
#adonis(vegdist(asv_tab.gt1K.rare, method = "bray", binary = T) ~ full_meatadata.sorted$Site)

#Neo occurence
simper.Nf = simper(
    comm = log10(asv_tab.gt1K.rare+1),
    group =  full_metadata.sorted$Nf,
    permutations = 999
)
summary(simper.Nf)
