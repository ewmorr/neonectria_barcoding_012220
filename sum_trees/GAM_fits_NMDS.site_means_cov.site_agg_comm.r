...
##########################
#min seqs per sample min#
##########################

#rarefied table
asv_tab.gtMin.rare = readRDS(file = "intermediate_RDS/asv_tab.gtMin.rare.site_sum.rds")
#NMDS
asv_tab.gtMin.rare.mds = readRDS(file = "intermediate_RDS/asv_tab.gtMin.rare.mds.site_sum.rds")


#Add metadata for ordisruf
asv_tab.gtMin.rare.mds.metadata = left_join(
data.frame(Site = rownames(asv_tab.gtMin.rare.mds$points), asv_tab.gtMin.rare.mds$points),
full_metadata, by = "Site")

asv_tab.gtMin.rare.mds.metadata$Site = factor(asv_tab.gtMin.rare.mds.metadata$Site, levels = c("MEN1", "MES1", "ADN1", "ADS1", "CW1", "TSP1", "GK1j", "ASH2", "MI1", "WF1"))

surf_vars = asv_tab.gtMin.rare.mds.metadata %>% select(starts_with("Site_mean"), duration_infection, tmax, MAT, tmin, ppt)

surf.dbh = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,1])
surf.neo_fruiting = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,2])
surf.cankers = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,3])
surf.tarry_spots = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,4])
surf.wax = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,5])
surf.xylococcus = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,6])
surf.tree_cond = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,7])
surf.dur_inf = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,8])
surf.tmax = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,9])#P = 0.00255
surf.MAT = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,10]) #ns
surf.tmin = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,11])#P = 0.00244
surf.ppt = ordisurf(asv_tab.gtMin.rare.mds ~ surf_vars[,12])
#all others P < 0.0001
...
p.dbh = ggplot() +
geom_point(data = asv_tab.gtMin.rare.mds.metadata, aes(MDS1, MDS2, fill = Site), size = 3.5, shape = 21) +
...
labs(title = "Sites", x = "NMDS1", y = "NMDS2") +
guides(fill = guide_legend(override.aes=list(shape=21,size=3)), shape = guide_legend(override.aes=list(size=3)))


pdf("GAM_fits/NMDS_neo_occurence_by_climate_GAM.sum_sites.pdf", width = 22, height = 12)
grid.arrange(p.site,p.dur_inf,p.tmin,p.MAT,p.tmax,p.ppt,nrow = 2)
dev.off()

pdf("GAM_fits/NMDS_neo_occurence_by_disease_severity_GAM.sum_sites.pdf", width = 22, height = 18)
...
#################################
#min seqs per sample min binary#
#################################

#rarefied table
asv_tab.gtMin.rare = readRDS(file = "intermediate_RDS/asv_tab.gtMin.rare.site_sum.rds")
#NMDS
asv_tab.gtMin.rare.mds.bin = readRDS(file = "intermediate_RDS/asv_tab.gtMin.rare.mds.bin.site_sum.rds")


#Add metadata for ordisruf
asv_tab.gtMin.rare.mds.bin.metadata = left_join(
data.frame(Site = rownames(asv_tab.gtMin.rare.mds.bin$points), asv_tab.gtMin.rare.mds.bin$points),
full_metadata, by = "Site")

asv_tab.gtMin.rare.mds.bin.metadata$Site = factor(asv_tab.gtMin.rare.mds.bin.metadata$Site, levels = c("MEN1", "MES1", "ADN1", "ADS1", "CW1", "TSP1", "GK1j", "ASH2", "MI1", "WF1"))

surf_vars = asv_tab.gtMin.rare.mds.bin.metadata %>% select(starts_with("Site_mean"), duration_infection, tmax, MAT, tmin, ppt)

surf.dbh = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,1])
surf.neo_fruiting = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,2])
surf.cankers = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,3])
surf.tarry_spots = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,4])
surf.wax = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,5])
surf.xylococcus = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,6])
surf.tree_cond = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,7])
surf.dur_inf = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,8])
surf.tmax = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,9])#P = 0.00255
surf.MAT = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,10]) #ns
surf.tmin = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,11])#P = 0.00244
surf.ppt = ordisurf(asv_tab.gtMin.rare.mds.bin ~ surf_vars[,12])
#all others P < 0.0001
...
p.dbh = ggplot() +
geom_point(data = asv_tab.gtMin.rare.mds.bin.metadata, aes(MDS1, MDS2, fill = Site), size = 3.5, shape = 21) +
stat_contour(data = surf.dbh.grid, aes(x = x, y = y , z = z, colour = (..level..)), size = 1) +
my_gg_theme +
theme(legend.title = element_text(size = 17, hjust = 0), legend.text = element_text(size = 15)) +
...
pdf("GAM_fits/NMDS.bin_neo_occurence_by_climate_BINARY_GAM.sum_sites.pdf", width = 22, height = 12)
grid.arrange(p.site,p.dur_inf,p.tmin,p.MAT,p.tmax,p.ppt,nrow = 2)
dev.off()

pdf("GAM_fits/NMDS.bin_neo_occurence_by_disease_severity_BINARY_GAM.sum_sites.pdf", width = 22, height = 18)
grid.arrange(p.site,p.dbh,p.neo_fruiting,p.wax,p.cankers,p.tarry_spots,p.xylococcus,p.tree_cond,nrow = 3)
dev.off()
